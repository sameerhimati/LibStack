...
## Next Session Should
1. **Opening gambit — iOS phone test of vault-bridge.** On your phone open `reading.itamih.com` → Settings → set **Worker URL** = `https://vault-bridge.sameerhimati98.workers.dev` and **Shared secret** = the `SHARED_SECRET` value you set on the worker (retrieve from your password manager — it is NOT in this repo or this file). Then run `PLAN-vault-bridge.md` → "Verification → PWA on phone": type a note (leave 6s, return — persists + commit in vault), tap mark-read (article drops off feed in ~60s), airplane-mode a note (→ pending sync → reconnect → synced), force-quit mid-pending (→ reopen flushes). This is the first real test that GitHub writes actually work (only the unauthed path is verified so far).
