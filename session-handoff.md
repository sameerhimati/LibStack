...
2. **Notes-vs-highlights pivot decision** (memory: `project_libstack_notes_pivot.md`). Sameer floated: "perhaps we remove notes and make it highlights only." Typing on phone is friction; highlights are more natural. Decide before/during Highlights v1.1 — they share infrastructure. If highlights replace notes, the existing `/api/notes` worker endpoint, IndexedDB queue, sheet UI, and build-time notes loader are mostly reusable (rename + adjust contract).

3. **Listen-to-article feature.** New thread from Sameer: "mark as read and then an option to listen to an article/post." MVP path: Web Speech API (`SpeechSynthesisUtterance`) — free, native iOS, no API key, decent voices. Per-article "Listen" button next to mark-read. Bigger version overlaps with the NotebookLM thread on the roadmap (auto-generate podcasts per cluster) — different scope, defer.

4. **Highlights v1.1** — still the cleanest "real" feature, but the notes-vs-highlights pivot might reshape it. If highlights replace notes, this becomes "rebuild the sheet around selection capture instead of textarea." Either way: spike iOS Safari selection on a real phone first (Safari's native callout fights custom `selectionchange`), then `POST /api/highlights` mirroring `/api/notes`, reusing IndexedDB queue.

5. Remaining roadmap polish (sort/filter, image localization, content-render polish, type controls, video tab, OG tags) — lower priority than the above.
